...
(async()=>{
 ...
 try{
  ...
  await a.locator('.friends-tile').click();await a.locator('#createName').fill('あおい');await a.locator('#panelCreate .btn.filled').click();await a.locator('#waiting.active').waitFor();const id=(await a.locator('#roomIdTxt').textContent()).trim();
  await b.locator('.join-tile').click();await b.locator('#joinName').fill('かえで');await b.locator('#joinId').fill(id);await b.locator('#panelJoin .btn.filled').click();await b.locator('#waiting.active').waitFor();await a.locator('#startBtn:not([disabled])').waitFor();await a.locator('#startBtn').click();await Promise.all([a.locator('#game.active').waitFor(),b.locator('#game.active').waitFor()]);
